...
func GetMerchantById(id uint64) (*Merchant, error) {
	ret := new(Merchant)
...
// 获取机构下面的商户
func QueryMerchantInAgency(agencyId, offset, limit uint64, filters map[string]string) (uint64, []*Merchant, error) {
	...
	// conditions
	tmpDb := models.DB.Model(&Merchant{}).Where(sqlCondition)
...
// 获取账号相关的商户列表
func QueryMerchantInUser(ctx *gin.Context, offset, limit uint64, filters map[string]string) (uint64, []*Merchant, error) {
	var ret []*Merchant
	...
	tmpDb := models.DB.Table("merchant").Model(&Merchant{}).Where(sqlCondition)
