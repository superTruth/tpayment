...
var iso8583Factory = &iso8583.Factory{Configs: [129]*iso8583Define.FieldConfig{
	...
	{FieldValueType: 1, FieldAlignType: 0, IsValueLenFix: true, ValueLen: 8, LenLen: 0, LenType: 1, PaddingByte: 0x0, Mask: false},     // F9
	...
	{FieldValueType: 1, FieldAlignType: 0, IsValueLenFix: true, ValueLen: 4, LenLen: 0, LenType: 1, PaddingByte: 0x0, Mask: false},     // F22
	...
	{FieldValueType: 1, FieldAlignType: 1, IsValueLenFix: false, ValueLen: 37, LenLen: 2, LenType: 1, PaddingByte: 0xff, Mask: false},  // F35
	...
	{FieldValueType: 2, FieldAlignType: 0, IsValueLenFix: false, ValueLen: 255, LenLen: 3, LenType: 1, PaddingByte: 0x0, Mask: false},  // F56
	...
	{FieldValueType: 0, FieldAlignType: 1, IsValueLenFix: false, ValueLen: 999, LenLen: 3, LenType: 1, PaddingByte: 0x0, Mask: false},  // F60
	{FieldValueType: 2, FieldAlignType: 1, IsValueLenFix: false, ValueLen: 999, LenLen: 3, LenType: 1, PaddingByte: 0x0, Mask: false},  // F61
	{FieldValueType: 0, FieldAlignType: 0, IsValueLenFix: false, ValueLen: 999, LenLen: 3, LenType: 1, PaddingByte: 0x0, Mask: false},  // F62
...
func SetCommonDataToMsg(req *acquirer_impl.SaleRequest, config *Config, fieldSelect []byte) (*iso8583.Message, error) {
	...
	for i := 0; i < len(fieldSelect); i++ {
		...
		case 2:
			if req.TxqReq.CreditCardBean.CardTrack2 == "" {
		...
		case 22:
			...
			if req.TxqReq.CreditCardBean.IsMsdCard {
				...
				case conf.ManualInput:
					entryMode.WriteString("501")
				case conf.Swipe:
					if !req.TxqReq.CreditCardBean.CardFallback {
						entryMode.WriteString("502")
					} else {
						switch req.TxqReq.RealPaymentMethod {
						case conf.MasterCard:
							entryMode.WriteString("580")
						case conf.Visa:
							entryMode.WriteString("590")
						case conf.JCB:
							entryMode.WriteString("597")
						default:
							entryMode.WriteString("580")
						}
					}
				case conf.Contact:
					entryMode.WriteString("505")
				case conf.ContactLess:
					entryMode.WriteString("507")
				default:
					return nil, errors.New("not support entry mode->" + req.TxqReq.RealEntryType)
				}
			} else {
				if req.TxqReq.RealPaymentMethod == conf.JCB {
					entryMode.WriteString("507")
				} else {
					entryMode.WriteString("591")
				}
			}
		...
		case 24:
			_ = iso8583Msg.SetFieldValue(24, config.AcquirerConfig.NII)
		case 26:
			if req.TxqReq.CreditCardBean.PIN != "" {
		...
		case 36:
			_ = iso8583Msg.SetFieldValue(36, req.TxqReq.CreditCardBean.CardTrack3)
		case 37:
