...
func (s *Sale) packageMsg() ([]byte, error) {
	...
	sendBytes, err := common.Encrypt(s.Req, s.Config, msg)
	if err != nil {
...
func (s *Sale) parseMsg(msgData []byte) (*acquirer_impl.SaleResponse, error) {
	plainData, err := common.Decrypt(msgData, s.Req)
	if err != nil {
		return nil, errors.New("decrypt fail->" + err.Error())
	}
	resp, _, err := common.GetCommonDataFromMsg(plainData, 0)
	if err != nil {
