...
// 打包
func ECDICFormat(config *iso8583Define.FieldConfig, value string) []byte {
	if config == nil || value == "" {
		return make([]byte, 0)
	}
	...
	// 写入数据
	valueTmp := format_utils.AppendString(value, fieldLen, config.FieldAlignType == iso8583Define.Right, config.PaddingByte)
	//valueTmp := make([]byte, fieldLen)
	//mix_utils.BytesFill(valueTmp, config.PaddingByte) // 数据填充
	//
	//valueBytes := []byte(*value)//convert_utils.ASCII2ECDIC(value)
	...
	//	break
	//}
	sb.Write(convert_utils.ASCII2ECDIC(&valueTmp))
	return sb.Bytes()
}

// 解包
func ECDICParse(config *iso8583Define.FieldConfig, buffer []byte, offset *int) (string, error) {

	bufferLen := len(buffer)

	if *offset < 0 || bufferLen == 0 {
		return "", errors.New("ECDIC parse fail")
	}
	...
	ret := convert_utils.ECDIC2ASCII(mix_utils.BytesArrayCopyArrange(buffer, *offset, (*offset)+valueLen))
